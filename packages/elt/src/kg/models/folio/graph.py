...
class FOLIO:
    ...
    def __init__(
        ...
        Args:
            source_type (str): The source type for loading the ontology. Either "github", "http", or "file".
            ...
            file_path (Optional[str | Path]): The path to the local OWL file when source_type is "file".
    ...
    def load_cache(
        ...
        if source_type == "github":
            ...
            if file_path is None:
                raise ValueError("File path must be provided for source type 'file'.")
            cache_key = str(Path(file_path).absolute())
        else:
            raise ValueError(
                "Invalid source type. Must be either 'github', 'http', or 'file'."
            )
    ...
    @staticmethod
    def load_owl_file(file_path: str | Path) -> str:
        """
        Load the FOLIO ontology in OWL format from a local file.

        Args:
            file_path (str | Path): The path to the local OWL file.

        Returns:
            str: The raw ontology buffer.
        """
        try:
            ...
            LOGGER.info("Loading ontology from local file: %s", file_path)
            with file_path.open("rt", encoding="utf-8") as f:
                return f.read()
        except Exception as e:
            raise RuntimeError(f"Error loading ontology from file {file_path}") from e
    ...
    def load_owl(
        ...
        if use_cache and source_type != "file":  # Skip cache for file source type
        ...
        if not owl_buffer:
            if source_type == "github":
                ...
                if file_path is None:
                    raise ValueError(
                        "File path must be provided for source type 'file'."
                    )

                # Load the ontology from a local file
                owl_buffer = FOLIO.load_owl_file(file_path=file_path)
            else:
                raise ValueError(
                    "Invalid source type. Must be either 'github', 'http', or 'file'."
                )

        # Save the ontology to the cache only if not file source type
        if use_cache and source_type != "file":
            FOLIO.save_cache(
                buffer=owl_buffer,
                source_type=source_type,
                http_url=http_url,
    ...
    def parse_owl_class(self, node: lxml.etree._Element) -> None:
        ...
        for (
            alt_label
        ) in owl_class.alternative_labels:  # pylint: disable=not-an-iterable
            if alt_label:
    ...
    def parse_owl_object_property(self, node: lxml.etree._Element) -> None:
        ...
        # normalize the IRI
        normalized_iri = self.normalize_iri(iri)

        # initialize the OWL object property
        owl_property = OWLObjectProperty(iri=normalized_iri)
    ...
    def get_triples_by_subject(self, subject: str) -> List[Tuple[str, str, str]]:
        ...
        """
        normalized_subject = self.normalize_iri(subject)
        return self._filter_triples(
            tuple(self.triples), normalized_subject, filter_by="subject"
        )
    ...
    def get_triples_by_predicate(self, predicate: str) -> List[Tuple[str, str, str]]:
        ...
        Returns:
            List[Tuple[str, str, str]]: The list of triples.
        """
        normalized_predicate = self.normalize_iri(predicate)
        return self._filter_triples(
            tuple(self.triples), normalized_predicate, filter_by="predicate"
        )
    ...
    def get_triples_by_object(self, obj: str) -> List[Tuple[str, str, str]]:
        ...
        Returns:
            List[Tuple[str, str, str]]: The list of triples.
        """
        normalized_object = self.normalize_iri(obj)
        return self._filter_triples(
            tuple(self.triples), normalized_object, filter_by="object"
        )
